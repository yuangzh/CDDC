...
for idata = 1:3
    ...
    myplot = @plot;
    myplot(One.ts1,One.his1,'-b+','LineWidth',5,'MarkerSize',3,'color',pcolor.yellow ); hold on;
    ...
    myplot(One.ts4,One.his4,':ks','LineWidth',4,'MarkerSize',3,'color', pcolor.red); hold on;
    myplot(One.ts5,One.his5,'-ro','LineWidth',4,'MarkerSize',3,'color', pcolor.green  ); hold on;

    hleg=legend('AltMin','GradProj','GPM','CD-SCA','CD-SNCA') ;
    ...
    axis([min(tss) max(tss) 0.97*min(hiss) 1.05*max(hiss)])
